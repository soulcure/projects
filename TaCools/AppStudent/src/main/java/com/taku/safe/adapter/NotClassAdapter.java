...
public class NotClassAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private Context mContext;
    private List<RespNoClassList.PageInfoBean.StudentlistBean> mList;

    public NotClassAdapter(Context context) {
        mContext = context;
        mList = new ArrayList<>();
    }

    public void setList(List<RespNoClassList.PageInfoBean.StudentlistBean> list) {
        if (list != null)
            mList.addAll(list);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        final RespNoClassList.PageInfoBean.StudentlistBean item = mList.get(position);
        ...
        String format = mContext.getResources().getString(R.string.days);

        tv_name.setText(item.getName());
        tv_phone.setText(item.getPhoneNo());
        tv_class.setText(item.getClassName());
        tv_info.setText(R.string.not_class);
        tv_days.setText(String.format(format, item.getDays()));

        view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(mContext, HardwareAnalysisDetailActivity.class);
                intent.putExtra(HardwareAnalysisDetailActivity.STUDENT_ID, item.getId());
                intent.putExtra(HardwareAnalysisDetailActivity.DETAIL_TYPE, HardwareAnalysisDetailActivity.NO_CLASS_DETAIL);
