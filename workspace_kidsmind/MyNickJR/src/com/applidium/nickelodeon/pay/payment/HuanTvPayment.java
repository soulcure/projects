...
public class HuanTvPayment {
    ...
    private boolean callPaySDK(final PayResponse payResponse) {
        ...
        vipName = payResponse.getSubject();  //产品名称
        ...
        sb.append('&').append("appPayKey").append('=').append(AppConfig.APP_PAYKEY);
        sb.append('&').append("productName").append('=').append(vipName);
            ...
            public void httpReqResult(String response) {
                ...
                if (statusResponse.isSuccess()) {
                    if (statusResponse.isTradeResult()) {
                        ((MNJApplication) mAct.getApplication()).reqUserInfo(mAct, vipName);
                    } else {
                        if (mQueryCount > 0) {
                            Message msg = mHandler.obtainMessage();
