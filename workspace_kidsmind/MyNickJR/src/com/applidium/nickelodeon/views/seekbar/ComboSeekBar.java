...
public class ComboSeekBar extends SeekBar {
    ...
    public ComboSeekBar(Context context, AttributeSet attrs) {
        ...
        mColor = a.getColor(R.styleable.ComboSeekBar_myColor, Color.RED);
        ...
        int top = getResources().getDimensionPixelOffset(R.dimen.comboseekbar_padding_top);
        int buttom = getResources().getDimensionPixelOffset(R.dimen.comboseekbar_padding_bottom);
        setPadding(0, top, 0, buttom);
    }
    ...
    public synchronized void setSelection(int position) {
        ...
        CustomDrawable drawable = (CustomDrawable) getProgressDrawable();
        if (drawable != null) {
            drawable.setCurIndex(position);
        }
    ...
    private void handleClick(Dot selected) {
        if ((prevSelected == null) || (prevSelected.equals(selected) == false)) {
            if (mItemClickListener != null) {
                mThumb.setThumbText(selected.text);
