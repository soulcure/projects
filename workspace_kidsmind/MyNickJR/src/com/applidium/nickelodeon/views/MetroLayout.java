...
public class MetroLayout extends FrameLayout implements View.OnFocusChangeListener {
    ...
    private int ITEM_NORMAL_SIZE_WIDTH;
    private int ITEM_NORMAL_SIZE_HEIGHT;
    ...
    private void init() {
        ...
        ITEM_NORMAL_SIZE_WIDTH = getResources().getDimensionPixelSize(R.dimen.ITEM_NORMAL_SIZE_WIDTH);
        ITEM_NORMAL_SIZE_HEIGHT = getResources().getDimensionPixelSize(R.dimen.ITEM_NORMAL_SIZE_HEIGHT);
    ...
    private View addItemView(View child, int celltype, int row, int padding) {
        ...
        switch (celltype) {
            ...
            case Horizontal:
                ...
                switch (row) {
                    ...
                    case ROW_SECOND:
                        if (mMirror) {
                            ...
                            flp.topMargin = getPaddingTop();
                            flp.rightMargin = getPaddingRight();
                            flp.topMargin += ITEM_H_HEIGHT + padding;
                            addView(mirror, flp);
                            result = mirror;
                        } else {
                            flp.leftMargin = rowOffset[1];
                ...
                break;
            case Normal:
                flp = new LayoutParams(ITEM_NORMAL_SIZE_WIDTH, ITEM_NORMAL_SIZE_HEIGHT);
                switch (row) {
                    case ROW_FIRST:
                        flp.leftMargin = rowOffset[0];
                        flp.topMargin = getPaddingTop();
                        flp.rightMargin = getPaddingRight();
                        addView(child, flp);
                        rowOffset[0] += ITEM_NORMAL_SIZE_WIDTH + padding;
                        break;
                    case ROW_SECOND:
                        if (mMirror) {
                            MirrorItemView mirror = new MirrorItemView(mContext);
                            ...
                            flp.topMargin = getPaddingTop();
                            flp.rightMargin = getPaddingRight();
                            flp.topMargin += ITEM_NORMAL_SIZE_HEIGHT + padding;
                            addView(mirror, flp);
                            result = mirror;
                        } else {
                            flp.leftMargin = rowOffset[1];
                        ...
                        rowOffset[1] += ITEM_NORMAL_SIZE_WIDTH + padding;
                        break;
                }
                break;
        }
