...
public class AutoLoginFragment extends Fragment {
    ...
    private class OnCreateProfileResult implements OnSucessListener {
        ...
        public void sucess() {
            ...
            if (bundle != null) {
                String action = bundle.getString(MyPushMessageReceiver.ACTION);
                if (action != null) {
                    if (action.equals(MyPushMessageReceiver.SERIE)) {
                        int serieId = bundle.getInt(MyPushMessageReceiver.SERIEID);
                        int sequence = bundle.getInt(MyPushMessageReceiver.SEQUENCEID);
                        if (serieId > 0) {
                    ...
                    } else if (action.equals(MyPushMessageReceiver.ACTIVITY)) {
                        Intent intent = new Intent(mAct, ActionActivity.class);
                        mAct.startActivityForResult(intent, KidsMindFragmentActivity.REQUEST_MODE_A);
                    } else if (action.equals(MyPushMessageReceiver.TOPIC)) {
                        int topicId = bundle.getInt(MyPushMessageReceiver.SERIEID);
                        Intent intent1 = new Intent(mAct, FreePlayingActivity.class);
                        intent1.putExtra("type", FreePlayingActivity.FROM_TOPIC_PLAY);
                        intent1.putExtra("serieId", topicId);
                        mAct.startActivityForResult(intent1,
                                KidsMindFragmentActivity.REQUEST_MODE_A);
                    }
                    return;
                }
