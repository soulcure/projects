...
public class CustomDigitalClock {
    ...
    public CustomDigitalClock(Context context) {
        ...
        mCountMillSecond = PlaySettingFragment.getLimitTime(context);  //获取默认的设置时间
    }
    ...
    public long getLimitTime(Context context) {
        ...
        if (time == defaultTime) {
            time = PlaySettingFragment.getLimitTime(context);  //获取默认的设置时间
        }
        return time;
    }
        ...
        if (isLimited) {
            if (!mTimeHandler.hasMessages(HANDLER_CLOCK)) {
                if (mCountMillSecond == 0) {
                    mCountMillSecond = getLimitTime(mContext);
                }
