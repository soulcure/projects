...
public class PlayerReportModel {
    ...
    public PlayerReportModel(Context context) {
        ...
        accountId = ((KidsMindApplication) context.getApplicationContext()).getUserId();
        ...
        promoterCode = ((KidsMindApplication) context.getApplicationContext()).getProperty("ChannelNo");
        ...
        mPlayReportRequest = new PlayReportRequest();
    }


    public void init(int profileId, int episodeId, String audioLang,int behaviorPlayId) {
        this.profileId = profileId;
        this.contentId = episodeId;
        this.audioLang = audioLang;
        mPlayReportRequest.setBehaviorPlayId(behaviorPlayId);
    ...
    public void addPlayTime(int time) {
        ...
        mPlayReportRequest.setPlayDuration(playTime / 1000);  //毫秒转换为秒
    }
    ...
    public void playStart() {
        ...
        mPlayReportRequest.setFirstBufferLen(0);
    ...
    public void playPrepared() {
        ...
        int temp = (int) (cur - curPlayTime) / 1000;
        if (temp < 60 && temp > 0) {
            startLatency = temp;
        } else {
            startLatency = 0;
        ...
        mPlayReportRequest.setStartTime(cur);
        mPlayReportRequest.setFirstBufferLen(startLatency);
    }
    ...
    private void playCompletion() {
        long cur = System.currentTimeMillis();
        mPlayReportRequest.setEndTime(cur);
        //int id = 0;//((KidsMindApplication) mContext.getApplicationContext()).getBehaviorId();
         //do something
        //mPlayReportRequest.setBehaviorPlayId(id);
    ...
    public void loadingStart() {
        ...
        PlayReportRequest.LoadingData item = new PlayReportRequest.LoadingData();
        item.lockStartTime = loadingTime;

        mPlayReportRequest.lockDataAdd(item);
    }
    ...
    public void loadingEnd() {
        long cur = System.currentTimeMillis();
        long sum = cur - loadingTime;
        if (sum > 0) {
            bufferingPauseTime += sum;
        }

        int last = mPlayReportRequest.getLockData().size() - 1;
        if (last >= 0) {
            PlayReportRequest.LoadingData data = mPlayReportRequest.getLockData().get(last);
            data.lockEndTime = cur;
        }
    }
    ...
    public void pauseStart() {
        ...
        PlayReportRequest.PauseData item = new PlayReportRequest.PauseData();
        item.pauseStartTime = pauseTime;
        mPlayReportRequest.pauseDataAdd(item);
    }
    ...
    public void pauseEnd() {
        long cur = System.currentTimeMillis();
        long sum = cur - pauseTime;
        if (sum > 0) {
            userPauseTime += sum;
        }

        int last = mPlayReportRequest.getPauseData().size() - 1;
        if (last >= 0) {
            PlayReportRequest.PauseData data = mPlayReportRequest.getPauseData().get(last);
            data.pauseEndTime = cur;
        }
    }
    ...
    public void setReqUrlTime() {
        long cur = System.currentTimeMillis();
        getUrlTime = (cur - getUrlTime) / 1000;

        mPlayReportRequest.setPlayUrlLen((int) getUrlTime);
    }
    ...
    public void setInfo(String info) {
        mPlayReportRequest.setInfo(info);
    }
    ...
    public void report() {
        ...
        String msg = "session=" + session + " & totalTime = " + totalTime / 1000 + " & playTime = " + playTime / 1000 + " & bufferingPauses = " + bufferingPauses;
        Log.v("colin", msg);
        if (AppConfig.debug) {
    ...
    private void playReport() {
        String url = AppConfig.PLAY_REPORT;

        String token = ((KidsMindApplication) mContext.getApplicationContext()).getToken();
        mPlayReportRequest.setToken(token);

        mPlayReportRequest.setEpisodeId(contentId);

        String json = mPlayReportRequest.toJsonString();
        HttpConnector.httpPost(url, json, new IPostListener() {

            @Override
            public void httpReqResult(String response) {
                mPlayReportRequest.clear();
            }
