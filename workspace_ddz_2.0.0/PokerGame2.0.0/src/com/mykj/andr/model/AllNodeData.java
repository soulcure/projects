...
public class AllNodeData {
	...
	private List<NodeData> mNodeDataList=new ArrayList<NodeData>();
	private List<TDataInputStream> mTdis=new ArrayList<TDataInputStream>();
	private List<NodeData> mFirstList = new ArrayList<NodeData>();
	private int mRootID=0;
	
	private int mTotal=0;  //静态数据
	
	/**A计划新增字段*/
	private long mSumPerson=0;//当前总人数   
	...
	/**当前获取的数据来源类型，对应下面3中data类型*/
	private int currentDataType = NODE_DATA_ERROR;
	...
	public int addTDataInputStream(TDataInputStream tdis){
		...
		currentDataType = NODE_DATA_SOCKET;
	...
	public void updateRoomData(TDataInputStream tdis){
		...
		tdis.readByte();//快速开始时，进入符合条件的默认玩法节点
		mSumPerson=tdis.readLong();//当前所有分区人数总和
		...
		for(int i=0;i<curCount;i++){
			...
			String exName=sn.getExName();
			String[] subAreaNodeTips=sn.getSubAreaNodeTips();
			...
			if(node!=null){
				...
				node.Name=name;
				node.ExName=exName;
				node.SubAreaNodeTips=subAreaNodeTips;
			}
	...
	List<List<NodeData>> mLList = new ArrayList<List<NodeData>>();

	private void initMatchList(){
		synchronized (mLList) {
			if(mLList.size() == 0){
				/*数据生成*/
				Map<String,List<NodeData>> mMap = new HashMap<String, List<NodeData>>();
				for(int i = 0;i< mNodeDataList.size();i++){
						if(mNodeDataList.get(i).Type == 109){
						String key = mNodeDataList.get(i).NameKey;
						List<NodeData> lst = mMap.get(key);
						if(lst == null){
							lst = new ArrayList<NodeData>();
						}
						lst.add(mNodeDataList.get(i));
						mMap.put(key, lst);
					}
				}
				for(String key:mMap.keySet()){
					mLList.add(mMap.get(key));
				}
			}
		}
	}
	
	public void updateMatchAttend(NodeData node){
		for(List<NodeData> subList : mLList){
			for(NodeData item : subList){
				if(item.ID == node.ID){
					item.setAttend(node.isAttend());
					return;
				}
			}
		}
	...
	public List<List<NodeData>> getMatchList(){
		return mLList;
	}
	...
	public List<NodeData> getCardZoneNodeDate(){
		...
		if(isRecFinish){
			NodeData nd=getQuickEntryNodeDate();
			//快速进入节点
			cardZoneList.add(nd);
			//推荐节点
			cardZoneList.add(getRecommendNodeDate(nd));
			//添加一级节点
			cardZoneList.add(getFirstCardNodeData());
		}
	...
	private NodeData getFirstCardNodeData(){
		NodeData node=null;
		List<NodeData> lists=getFirstNodeDate();
		for(NodeData item:lists){
			if(item.Recommend==1){
				node=item;
				break;
			}
	...
	public List<NodeData> getCardZoneNodeDate(final byte playId){
		...
		if(isRecFinish){
			...
			cardZoneList.add(nd);
			//推荐节点
			cardZoneList.add(getRecommendNodeDate(nd,playId));
			//添加一级节点
	...
	private NodeData getFirstCardNodeData(final byte playId){
		NodeData node=null;
		List<NodeData> lists=getFirstNodeDate(playId);
		for(NodeData item:lists){
			if(item.Recommend==1 && item.PlayID==playId){
				node=item;
				break;
			}
	 ...
	 * @return
	 */
	public NodeData getMMVideoNodeData(final byte playId){
		NodeData node=null;
			List<NodeData> allNode=getAllNodeDate();
			for(NodeData item:allNode){
				if(item.ParentID!=mRootID 
						&& item.Type==NodeData.NODE_MM_VIDEO
