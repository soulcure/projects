...
public class UnipayPayment {
	...
	// private static final String APPID = "904516268520130319114019266400";//
	// 应用编号
	...
	public void initPayment() {
		...
		int type = 1;
		Utils.getInstances().initSDK(mContext, new PayResultListener());
	}
...
//				PayManager.PAY_SIGN_UNICOM_UNIPAY);
		...
		if (number == null || "".equals(number)) {
			Toast.makeText(mContext, "支付参数有误：number=" + number,
					Toast.LENGTH_LONG).show();
			return;
		}

		if (orderid == null || "".equals(orderid)) {
			Toast.makeText(mContext, "支付参数有误：orderid=" + orderid,
					Toast.LENGTH_LONG).show();
			return;
		}
		currentShopID = shopID;
		/*
		 * 参数顺序：Context对象；计费点编号(的后三位)；计费点类型(SubMonth(按次代缴/真包月订购),UnSubMonth(按次代缴/
		 * 真包月退订),Other(道具，关卡))；支付订单号；支付接口回调接口。 number="007";
		 */
		Utils.getInstances().payOnline(mContext, number,"1", orderid,
	...
	public class PayResultListener implements UnipayPayResultListener {
		...
		public void PayResult(String code, int flag, int flag1, String error) {
			...
			case Utils.SUCCESS:
				// 此处放置支付请求已提交的相关处理代码
				PayUtils.SDKOrderOK(mContext, currentShopID,
						PayManager.PAY_SIGN_UNICOM_UNIPAY);
				break;
