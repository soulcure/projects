...
public class AlixPayment {
	...
	public void Analytical(int shopID, String signParam) {
		if (!Util.isEmptyStr(signParam)) {
			CallPayment(shopID, signParam);

			PayManager.getInstance(mContext).netReceive(shopID,
					PayManager.PAY_SIGN_ALIPAY);
		} else {
	 ...
	 * 调用支付SDK
	 */
	private void CallPayment(final int shopID, final String signParam) {
	...
	Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case RQF_PAY:
			case RQF_LOGIN:
				if (result.result_code.equals("4000")
						|| result.result_code.equals("6001")) {
