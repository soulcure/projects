...
public class LoginGiftDialog extends AlertDialog implements
	...
	public void onGiftResult(int result,String note) {
		Message msg = hand.obtainMessage(HANDLER_GIFT_RESULT);
		msg.arg1 = result;
		msg.obj = note;
		msg.sendToTarget();
	}
	...
	public void onStatusReceive(){
		hand.sendEmptyMessage(HANDLER_STATUS_RECEIVE);
	}
	...
	private void signResult(byte result) {
		if (result > 0) {
			updateInfo();
			if (result != 32) {
				LoginGiftSuccessDialog lsDlg= new LoginGiftSuccessDialog(ctx, result);
				lsDlg.setParent(LoginGiftDialog.this);
				lsDlg.show();
			}
		}
	}

	private void giftResult(final int result, String note) {
		updateInfo();
...
//		Util.displayCenterToast(findViewById(R.id.forget_btn), note);
		String confirm = result == 0?"好的":"知道了";
		CustomDialog dlg = new CustomDialog(ctx, note,confirm);
		dlg.setConfirmCallBack(new View.OnClickListener() {
			
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if(result == 0){
					Intent intent = new Intent(ctx,ExchangeTabActivity.class);
					intent.putExtra("gotoPage", ExchangeTabActivity.TO_BACKPACK);
	...
	private void updateInfo() {
		...
		if (status.unsignNum == 0) { // 没有漏签
			...
			((Button)findViewById(R.id.forget_btn)).setTextColor(0xffcdcdcd);
		} else {
			findViewById(R.id.forget_btn).setEnabled(true);
			((Button)findViewById(R.id.forget_btn)).setTextColor(0xff08620e);
		}
	...
	private void resign() {
		...
		if (status.propCount <= 0) {
			LoginGiftQuickItem buyItem = LoginGiftManager.getInstance()
					.getBuyItem();
			if (buyItem != null) {
				PayUtils.showBuyDialog(ctx, buyItem.id, FastBuyModel.isFastBuyConfirm,"",""); 
			}
		} else {
	...
	Handler hand = new Handler() {
		...
		public void dispatchMessage(Message msg) {
			...
			if (what == HANDLER_SIGN_RESULT) {
				...
				giftResult(msg.arg1 ,(String) msg.obj);
			}else if(what == HANDLER_STATUS_RECEIVE){
				updateInfo();
			}
		}
	};
