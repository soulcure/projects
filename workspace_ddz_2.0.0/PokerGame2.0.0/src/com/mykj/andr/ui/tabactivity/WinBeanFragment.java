...
public class WinBeanFragment extends ListFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		winBeanUrl = getUrl() + sign;
		
		new WinBeanAsyncTask().execute(winBeanUrl);
		...
		item.userVipLevel = (TextView)rootView.findViewById(R.id.user_vip_level);
	...
	private String getUrlParam(){
        ...
        sb.append('&').append("rid=").append(15);
				...
				while (eventType != XmlPullParser.END_DOCUMENT) {
					...
					case XmlPullParser.START_TAG:
						...
						if (itemInfo != null) {
							...
							}else if(tagName.equals("vipid")){
								itemInfo.vipId = Byte.parseByte(p.nextText());
							}else if(tagName.equals("is_expires")){
								itemInfo.isExpires = Boolean.parseBoolean(p.nextText());
							}
						}
		...
		protected void onPostExecute(Integer result) {
			...
			if(result == 0){
				if(!itemInfos.isEmpty()){
					...
					WinBeanFragment.this.setListAdapter(new WinBeanAdapter(mAct, itemInfos,mSetImageViewListener));
					...
					if(curUserRank > 50 || curUserRank <= 0){ // only show the first 50 players
						item.rankId_2.setImageDrawable(mResources.getDrawable(R.drawable.no_rank));
						item.rankId_1.setImageDrawable(null);
					}else{
						// 显示当前用户的排行
						mSetImageViewListener.setImageViewDrawable(item.rankId_2, item.rankId_1, curUserRank);
					...
					UtilHelper.setVipView(item.userVipLevel, user.getVipLevel(), !user.isVip());
					item.btnToWin.setText(mResources.getString(R.string.win_bean_tip));
					item.btnToWin.setOnClickListener(new View.OnClickListener() {
					...
					tip.setText(mResources.getString(R.string.cur_rank_info_empty));
				}
