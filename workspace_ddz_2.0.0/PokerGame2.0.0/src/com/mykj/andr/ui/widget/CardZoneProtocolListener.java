...
//                            FiexedViewHelper.setSavedMessage(savedMsg);
                ...
                case HANDLER_TYPE_COMMOM_POP:
                    ...
                    if ((event & SystemMessage.TYPE_EVENT_FIRST_LOGINED) == SystemMessage.TYPE_EVENT_FIRST_LOGINED) {
                    	DialogMsgQueue.getInstance().add(DialogMsgQueue.DIALOG_WELCOME);
                        DialogMsgQueue.getInstance().add(DialogMsgQueue.DIALOG_FIRST_LOGIN, content);
                        LoginGiftSuccessDialog.showBeVip = false;
                    }else if((event & SystemMessage.TYPE_EVENT_OLD_FIRST_LOGINED) == SystemMessage.TYPE_EVENT_OLD_FIRST_LOGINED){
                    	DialogMsgQueue.getInstance().add(DialogMsgQueue.DIALOG_OLD_FIRST_LOGIN, content);
...
//            CustomDialog dlg= UtilHelper.showCustomDialog(mAct, quickMessage, new OnClickListener() {
//                @Override
//                public void onClick(View v) {
//                	FiexedViewHelper.getInstance().playKeyClick();
//                    //****************************************
...
//                }
//            });
//            dlg.setConfirmStr("挑战更高");
            MeinvDialog mDlg = new MeinvDialog(mAct, quickMessage, "挑战更高", MeinvDialog.MEINV_TYPE_KAWAYI);
            mDlg.setConfirmCallBack(new OnClickListener() {
                @Override
                public void onClick(View v) {
                	FiexedViewHelper.getInstance().playKeyClick();
                    //****************************************
