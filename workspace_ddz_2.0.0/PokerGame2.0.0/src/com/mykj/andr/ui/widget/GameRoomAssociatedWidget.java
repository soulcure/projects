			...
			public boolean doReceive(NetSocketPak netSocketPak) {
				...
				if (FiexedViewHelper.getInstance().getCurFragment() != FiexedViewHelper.COCOS2DX_VIEW) {
					...
					if (socketUserInfo.userID == myUserID) { // 本尊
						...
						byte MyStatus = socketUserInfo.userStatus;
...
//							myuserInfo.setValue(socketUserInfo); // 更新自己的用户数据
							myuserInfo.userStatus = socketUserInfo.userStatus; //只更新状态，其他不更新
							myuserInfo.nickName = socketUserInfo.nickName; //游戏内修改昵称，更新分区结构数据
							UserCenterData userData = UserCenterProvider.getInstance()
					                .getUserCenterData();
					        if (userData != null) {
					        	userData.winBout = socketUserInfo.winCount;
					        	userData.loseBout = socketUserInfo.lostCount;
					        	userData.masterScore = socketUserInfo.masterScore;
					        }
							myuserInfo.winCount = socketUserInfo.winCount;
							myuserInfo.lostCount = socketUserInfo.lostCount;
							myuserInfo.masterScore = socketUserInfo.masterScore;
