...
public class ActionAdapter extends BaseAdapter{
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (row == null) {
			...
			holder.actionIcon = (Button) row.findViewById(R.id.ivActionjoin);//参与活动Button
		...
		if (info != null) {
			...
			holder.actionIcon.setOnClickListener(new OnClickListener() {
				private String url =info.url;
				private int  uo = info.uo;
				@Override
				public void onClick(View v) {
					FiexedViewHelper.getInstance().playKeyClick();
					if (!Util.isEmptyStr(url)) {
						 url = url.contains("?") ? url + "&" : url + "?";
						 UtilHelper.onWeb(mAct, CenterUrlHelper.getUrl(
							url, HallDataManager.getInstance()
									.getUserMe().userID), uo);
					}
					 int key = 65+position;
					 String keyString = "0"+String.valueOf(key);
					 AnalyticsUtils.onClickEvent(mAct, keyString);//参加活动
				}
			});
			//			if (icon.equals("0")) {
				...
				if (drawableId > 0) { // res有图片
					...
					if(file.exists()){
						...
						if(bitmap!=null){
							...
							file.delete();
							holder.actionImg.setImageResource(R.drawable.replace_icon);
