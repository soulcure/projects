...
public class NodeData implements Serializable {
	...
	/**新平台A计划添加字段*/
	public byte PicID; //图片ID

	//public byte ExNameLen;  //副标题长度
	public String ExName;     //副标题

	public byte RoomTagCount;  //房间节点标签数 （数据只有SubType = 1时才会被填充）
	public RoomTag[] RoomTags; //房间标签

	public byte SubAreaNodeTipCount = 0;  //分区提示个数
	...
	public String NameKey; //2.0.0新加，比赛场节点分类
	public String PicName; //2.0.0新加，列表图片
	...
	public NodeData(TDataInputStream dis){
		...
		//if(!AppConfig.isOuterNet()){
			...
			PicID=dis.readByte(); //图片ID

			byte ExNameLen=dis.readByte();  //副标题长度
			ExName=dis.readUTF(ExNameLen);     //副标题
			...
			SubAreaNodeTipCount=dis.readByte();  //分区提示个数
		...
		NameKey = dis.readUTFByte();
		PicName = dis.readUTFByte();
	...
	private boolean canEnter(UserInfo user) {
		...
		for (int i = 0; i < limits.length; i++) {
			...
			/*else if (limits[i].Type == 2) {// 元宝
				...
				if (user.getBean() >= limits[i].Min && user.getBean() <= limits[i].Max) {
					return true;
				}
	...
	public String getSubAreaNodeTip(){
		if(SubAreaNodeTips == null || SubAreaNodeTips.length == 0){
			return null;
		}
		
		return SubAreaNodeTips[(new Random()).nextInt(SubAreaNodeTipCount)];
	}
