...
public class CardZoneDataListener {
		...
		NetPrivateListener nPListener = new NetPrivateListener(parseProtocol) {
			...
			public boolean doReceive(NetSocketPak netSocketPak) {
				...
				if(AllNodeData.getInstance(mAct).isSubRecFinish()){
					...
					//如果UI已经在卡片分区，收到此协议数据，代表比赛场节点有数据更新
					if(FiexedViewHelper.getInstance().getCurFragment()==FiexedViewHelper.CARDZONE_VIEW){
						msg.what=REFRESH_MATCH_NODE_DATA;
					}else{
						if (AllNodeData.getInstance(mAct).getCurrentDataType() == AllNodeData.NODE_DATA_LOCAL
								&& isFirstRec) {
	...
	public void registerAttendListener(){
		// 定义接受数据的协议
		short[][] parseProtocol = { { MDM_LOBBY, MSUB_CMD_NODE_APPLY_STATUS } };
		NetPrivateListener nPListener = new NetPrivateListener(parseProtocol) {
			@Override
			public boolean doReceive(NetSocketPak netSocketPak) {
				// 解析接受到的网络数据
				TDataInputStream tdis = netSocketPak.getDataInputStream();
				int nodeId = tdis.readInt();
				byte status = tdis.readByte();
				FiexedViewHelper.getInstance().sHandler.obtainMessage(FiexedViewHelper.HANDLER_ATTEND_STATUS, nodeId, status).sendToTarget();
				return true;
			}
		};
		NetSocketManager.getInstance().addPrivateListener(nPListener);
